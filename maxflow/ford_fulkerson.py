...
from graph.shortest_path import dijkstra, dijkstra_pq


def ford_fulkerson(flow_network):
    flow_network.reset()

    get_path = lambda: dijkstra(flow_network.residual, flow_network.source, flow_network.sink)
    path = get_path()
    while len(path):
...
def ford_fulkerson_pq(flow_network):
    flow_network.reset()

    get_path = lambda: dijkstra_pq(flow_network.residual, flow_network.source, flow_network.sink)
    path = get_path()
    while len(path):
        path_arcs = [(path[i], path[i+1]) for i in range(len(path)-1)]
        min_capacity = flow_network.residual.get_arc_capacity(
